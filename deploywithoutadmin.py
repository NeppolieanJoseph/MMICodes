...
def mmi_deploy():
	...
	#	db_pass_txt.grid(row=8, column=2, sticky='NS')
		db_pass_txt.place(x=150,y=295,width=365,height=35)
	...
	#	log_location_txt.grid(row=10, column=2, sticky='NS')
		log_location_txt.place(x=150,y=360,width=365,height=35)
		...
		C2 = Checkbutton(window, text = "Secure Server", variable = CheckVar2, \
					onvalue = 1, offvalue = 0, height=5, bg='grey', \
					width = 50)
		C2.place(x=143,y=102,width=100,height=15)
...
#		log_location=log_location.replace( '\\', '\\\\' )
	...
	def main():
		...
		try:
			...
			print '\n'+tomcat_home+' is not available in this machine please update correct version in the config.yaml file OR You should run the program in run as admin mode'
			copy_token_file()
			delete_unwanted_in_workspace()
			exit()
		except (KeyboardInterrupt) as e:
